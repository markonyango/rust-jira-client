...
pub async fn add_watcher(configuration: &configuration::Configuration, issue_id_or_key: &str, body: &str) -> Result<serde_json::Value, Error<AddWatcherError>> {
    ...
    let local_var_uri_str = format!("{}/rest/api/2/issue/{issueIdOrKey}/watchers", local_var_configuration.base_path, issueIdOrKey=crate::apis::urlencode(issue_id_or_key));
...
pub async fn get_is_watching_issue_bulk(configuration: &configuration::Configuration, issue_list: models::IssueList) -> Result<models::BulkIssueIsWatching, Error<GetIsWatchingIssueBulkError>> {
    ...
    let local_var_uri_str = format!("{}/rest/api/2/issue/watching", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
