...
pub async fn get_status(configuration: &configuration::Configuration, id_or_name: &str) -> Result<models::StatusDetails, Error<GetStatusError>> {
    ...
    let local_var_uri_str = format!("{}/rest/api/2/status/{idOrName}", local_var_configuration.base_path, idOrName=crate::apis::urlencode(id_or_name));
...
pub async fn get_statuses(configuration: &configuration::Configuration, ) -> Result<Vec<models::StatusDetails>, Error<GetStatusesError>> {
    ...
    let local_var_uri_str = format!("{}/rest/api/2/status", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetStatusesError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
