...
/// TransitionUpdateDto : The transition update data. Note that a transition can have either the deprecated `to`/`from` fields or the `toStatusReference`/`links` fields, but never both nor a combination.
...
pub struct TransitionUpdateDto {
    ...
    pub from: Option<Vec<models::StatusReferenceAndPort>>,
    ...
    pub to: Option<Option<Box<models::StatusReferenceAndPort>>>,
